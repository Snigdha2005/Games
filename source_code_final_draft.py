...
def clue_wise_solve4(grid_row : list[str], single_line_clue : list[str]) -> list[str]:
    clue = ["4", "22", "13", "31"]
...
def clue_wise_solve3(grid_row : list[str], single_line_clue : list[str]) -> list[str]:
    clue = ["3", "12", "21", "111"]
...
def row_wise_initial_solve(grid : list[list[str]], row_clues : list[list[int]]) -> list[list[str]]:
    for pos, single_line_clue in enumerate(row_clues):
        if sum(single_line_clue) == GRID_ORDER:
            ...
            grid[pos] = clue_wise_solve4(grid[pos], int_to_str(single_line_clue))
        elif sum(single_line_clue) == GRID_ORDER - 2:
            grid[pos] = clue_wise_solve3(grid[pos], int_to_str(single_line_clue))
    return grid
...
def after_initial_solving(grid : list[list[str]], single_line_clue : list[str]) -> list[list[str]]:
    clue = ["2", "11"]
    for i in grid:
        if single_line_clue in clue:
            if i.count(BLACK) == 2 :
...
def row_final_solving(grid : list[list[str]], row_clues : list[list[int]]) -> list[list[str]]:
    for pos, single_line_clue in enumerate(row_clues):
        grid = after_initial_solving(grid, int_to_str(single_line_clue))
    return grid
...
def combine_gridsC(column_grid : list[list[str]], row_grid : list[list[str]]) -> list[list[str]]:
...
def computer_solve(row_clues : list[list[int]], column_clues : list[list[int]]) -> list[list[str]]:
    ...
    grid = combine_gridsC(column_final_solving(grid, column_clues), row_final_solving(grid, row_clues))
    return grid
